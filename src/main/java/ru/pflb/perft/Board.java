    ...
    private void makeCapture(Move move, byte[][] piecePos, Color color) {
        for (int i = 0; i < piecePos[color.code].length; i++) {
            if (piecePos[color.code][i] == val(move.to)) {
                if (piecePos[color.code][i + 1] == 0) {
                    piecePos[color.code][i] = 0;
                } else {
                    piecePos[color.code][i] = piecePos[color.code][i + 1];
                    piecePos[color.code][i + 1] = 0;
                }
        ...
        if (move.isCapture()) {
            ...
            switch (move.capture.get()) {
                ...
                case B_KNIGHT:
                    ...
                    break;
                default:
                    throw new IllegalStateException("Неправильное взятие");
            }
